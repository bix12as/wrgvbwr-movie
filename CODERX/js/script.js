      ...
      results.forEach((result) => {
        if (result.i && (result.qid === "movie" || result.qid === "tvSeries")) {
          ...
          if (result.qid === "movie" && result.i) {
            imageAndInfo = `<a onClick="setUrl(this); return setVideo(this);" url="coderx&type=movie&title=${result.l.replace(/ /g, "_")}" isWebSeries="false" title="${result.l}"  class="links" IMDB="${result.id}" href="https://www.2embed.cc/embed/${result.id}" target="_blank">
            ...
            imageAndInfo = `<a onClick="setUrl(this); return setVideo(this);" url="coderx&season=1&episode=1&title=${result.l.replace(/ /g, "_")}" IMDB="${result.id}" title="${result.l}" isWebSeries="true" class="links" href="https://www.2embed.cc/embedtv/${result.id}&s=1&e=1" target="_blank">
                      <img alt="${result.l}" src="${optimisedImageUrl(result.i.imageUrl)}">
                        <div class="info">
                          <h3>${result.l}</h3>
                          <p>${result.s}</p>
...
function setAll(imdb, title, season, episode, type) {
  if (imdb && title && !season && !episode && type) {
    ...
    a.setAttribute(
      "url",
      `&type=movie&title=${title.replace(/ /g, "_")}`
    );
    ...
    a.setAttribute("url", `coderx&season=${season}&episode=${episode}`);
    ...
    async function printEpisodes() {
      ...
      for (
        ...
        for (const episode of episodesDataJSON.episodes) {
          ...
          episodesData += `<a class="episodes" title="${seasonsDataJSON.name + ": E" + formatedEpisodeNumber + ". " + episode.name}" cssidentification="s${seasonNumber}e${episodeNumber}" url="coderx&season=${seasonNumber}&episode=${episodeNumber}&title=${seasonsDataJSON.name.replace(/ /g, "_") + "_E" + formatedEpisodeNumber + "_" + episode.name.replace(/ /g, "_")}" onClick="event.preventDefault();setVideo(this);setUrl(this); " href="https://www.2embed.cc/embedtv/${imdbID}&s=${seasonNumber}&e=${episodeNumber}">E${formatedEpisodeNumber}. ${episode.name}</a>`;
        }
